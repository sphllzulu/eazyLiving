...
import { useDispatch, useSelector } from 'react-redux';
import { auth, db, storage } from './firebase'; // Add storage from firebase
import { signOut } from 'firebase/auth';
...
const Profile = () => {
  const [profilePicture, setProfilePicture] = useState(null); // Holds the local image URL
  ...
  const fetchProfileData = async (uid) => {
    try {
      ...
      // Assuming profile picture URL is stored in Firestore
      const profilePicUrl = 'path/to/profilePicture.jpg'; // Placeholder URL
      setProfilePicture(profilePicUrl);
    } catch (error) {
      console.error('Error fetching profile data:', error);
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      ...
      const imageUrl = URL.createObjectURL(file); // Create a local image URL to display immediately
      setProfilePicture(imageUrl); // Display the selected image in the Avatar
    }
  };
  ...
  const handleImageUpload = async () => {
    ...
    const storageRef = ref(storage, `profilePictures/${user.uid}`); // Firebase Storage path
